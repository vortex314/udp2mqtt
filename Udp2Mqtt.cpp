...
void Udp2Mqtt::setConfig(Config config) { _config = config; }



void Udp2Mqtt::setLogFd(FILE* logFd) { _logFd = logFd; }
...
void Udp2Mqtt::udpSend(const string line) {
	...
	udpMsg.dstIp(_ipAsString.c_str());
	udpMsg.message=line;
...
Erc Udp2Mqtt::mqttConnect() {
	...
	INFO(" MQTT connecting %s ... for %s ", connection.c_str(), _ipAsString.c_str());
