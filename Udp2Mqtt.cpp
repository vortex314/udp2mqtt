		...
		switch(s) {
			...
			case UDP_RXD: {
					...
					if(_outgoing.size()) {
						...
						_outgoing.pop_front();
						INFO("[%X] UDP RXD : %s ",this,udpMsg.message.c_str());
...
void Udp2Mqtt::udpPublish(string topic, Bytes message, int qos, bool retained) {
	...
	udpSend(line);
	INFO("UDP TXD %s : %s ", _ipAsString.c_str(), line.c_str());
}
...
void Udp2Mqtt::mqttConnectionState(MqttConnectionState st) {
	INFO("[%X] MQTT connection state %s => %s ",this, mqttConnectionStates[_mqttConnectionState], mqttConnectionStates[st]);
...
Erc Udp2Mqtt::mqttConnect() {
	...
	INFO("[%X] MQTT connecting %s ... for %s ",this, connection.c_str(), _mqttClientId.c_str());
	if ( rc=MQTTAsync_create(&_client, connection.c_str(), _mqttClientId.c_str(), MQTTCLIENT_PERSISTENCE_NONE, NULL) !=MQTTASYNC_SUCCESS) {
		WARN("MQTTAsync_create() failed");
	}

	if ( rc=MQTTAsync_setCallbacks(_client, this, onConnectionLost, onMessage, onDeliveryComplete) != MQTTASYNC_SUCCESS) {
		WARN("MQTTAsync_setCallbacks() failed");; // TODO add ondelivery
	}
...
void Udp2Mqtt::onConnectFailure(void* context, MQTTAsync_failureData* response) {
	Udp2Mqtt* me = (Udp2Mqtt*)context;
	WARN("[%X] onConnectFailure : response.code : %d : %s ",me, response->code, response->message);
